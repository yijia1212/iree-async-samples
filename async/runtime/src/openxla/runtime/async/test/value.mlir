...
module @async{
  func.func @main() -> i32 {
    %0 = call @asynctest.return.async.scalar() : () -> !async.value
    %1 = call @async.value.await.i32(%0) : (!async.value) -> i32
    %2 = call @asynctest.return.async.scalar() : () -> !async.value
  ...
  func.func private @asynctest.return.async.scalar() ->!async.value
}
